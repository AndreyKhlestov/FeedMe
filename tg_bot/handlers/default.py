...
async def command_otchet(message: types.Message):
    ...
    markup.add(InlineKeyboardButton(text='Заполнить отчет', web_app=WebAppInfo(url=f'https://unique-leopard-enhanced.ngrok-free.app/telegram/report/{message.from_user.id}/'))) #
    return message.answer('Нажмите на кнопку для заполнения отчета', reply_markup=markup.as_markup())
