...
from admin_panel.telegram.models import Report, TradingPoint
...
class MailingForm(forms.Form):
    ...
    message_text = forms.CharField(
        ...
        help_text='Если у вас есть медиа, текст рассылки не должен превышать '
                  '1024 символов, без медиа - 4096'
    )
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name, field in self.fields.items():
            class_bootstrap = 'form-control' \
                if field_name != 'schedule_checkbox' else 'form-check-input'
    ...
    def clean(self):
        ...
        if not media_type:
            ...
            self.add_error(
                'schedule_datetime', 'Не указано время рассылки')
        elif media_type == 'no_media' and len(message_text) > 4096:
            self.add_error(
                'message_text',
                'Длина текста рассылки не должна превышать 4096 символов')
        elif media_type != 'no_media' and len(message_text) > 1024:
            self.add_error(
                'message_text',
                'Длина текста рассылки с медиа не должна превышать '
                '1024 символа')
        elif file:
            file_type = file.content_type.split('/')[0]
            if ((media_type == 'photo' and file_type != 'image') or
                    (media_type == 'video' and file_type != 'video') or
...
class ReportForm(forms.ModelForm):
    trading_point = forms.ModelChoiceField(queryset=TradingPoint.objects.all(),
                                           label='Торговая точка')
    images = forms.FileField(
        widget=forms.ClearableFileInput(
            attrs={"allow_multiple_selected": True}), required=False)
    wet_cats = forms.IntegerField(label="Влажный корм для кошек (кг)",
                                  min_value=0, required=True)
    dry_cats = forms.IntegerField(label="Сухой корм для кошек (кг)",
                                  min_value=0, required=True)
    wet_dogs = forms.IntegerField(label="Влажный корм для собак (кг)",
                                  min_value=0, required=True)
    dry_dogs = forms.IntegerField(label="Сухой корм для собак (кг)",
                                  min_value=0, required=True)
